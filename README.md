...
- **고를 것이 네 가지뿐**: 시장, 종목 하나(대표 개별 종목 또는 ETF, 코인은 5종), 기간, Jev에게 묻는 방식 하나를 라디오로 고릅니다. 종목코드를 직접 입력하지 않습니다. 시장별 기본 자본금과 수수료·세금은 미리 설정돼 있습니다.
...
| 화면 이름 | Jev 질문 타입 | 오른다 쪽 확률 | 내린다 쪽 확률 | 행동할 때 목표 비중 |
|---|---|---|---|---|
| 단호하게 | choice (buy/hold/sell) | 매수 확률 | 매도 확률 | 전부 사거나 전부 판다 |
| 확률대로 | choice (buy/hold/sell) | 매수 확률 | 매도 확률 | 매수 확률 + 보유 확률의 절반 |
| 오를까? | noul ("오를까?") | 오를 확률 | 1 − 오를 확률 | 전부 사거나 전부 판다 |
| 등급으로 | score (강력매도~강력매수 5단계) | 매수 + 강력매수 확률 | 매도 + 강력매도 확률 | 기대 등급을 0~100%로 환산 |

**확신 기준(임계값)**: 사용자가 95%, 90%, 85%, 80%, 75%, 70% 중 하나를 고릅니다(기본 80%). 오른다 쪽 확률이 기준 이상이면 삽니다. 파는 규칙은 두 가지입니다.

| 매도 규칙 | 동작 | 특징 |
|---|---|---|
| 반대 확신 매도 (기본) | 내린다 쪽 확률이 기준 이상일 때만 판다. 그 사이는 보유 유지 | 거래가 적고 수수료가 덜 든다 |
| 확신 떨어지면 즉시 매도 (고급 설정) | 오른다 쪽 확률이 기준 밑으로 내려가면 바로 판다 | 높은 기준에서는 샀다 팔았다를 반복하기 쉽다 |

기준이 높을수록 거래가 드물어집니다. 95%를 고르면 기간 내내 한 번도 사지 않을 수 있습니다. 임계값 도입 전 실행은 랭킹과 전략 분석에서 "이전 규칙"으로 따로 표시합니다.
