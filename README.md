...
- **Vercel**: `npm run build:vercel`이 Build Output API 산출물(화면 + API 함수 + 크론)을 만듭니다. 바이낸스 API가 미국 IP를 막기 때문에 함수는 서울(icn1) 리전에서 실행합니다. 배포 환경은 원본 틱을 저장하지 않고, 틱 체결을 고르면 체결일의 바이낸스 파일을 스트리밍으로 받아 주문이 채워지는 지점까지만 읽고 체결가를 계산합니다. 결과는 Turso에 캐시하며, 한 실행의 체결일은 최대 400개(코인·일)입니다. 백테스트는 응답 후 `waitUntil`로 끝까지 실행합니다. 함수가 시간 초과로 끊긴 실행은 크론이 다시 돌립니다.
...
| 원본 틱 (코인 기본값) | 코인 | 다음 날 0시부터 실제 체결을 순서대로 따라가며, 시장 체결량의 10%(`TICK_PARTICIPATION`)만 내 주문이 가져간다고 보고 계산 |

틱이 없으면 1분봉, 1분봉도 없으면 일봉 평균가로 대체하고, 결과에 출처별 체결 건수를 남깁니다. 체결 방식을 지정하지 않으면 코인은 원본 틱, 주식은 다음 날 시가로 체결합니다.
