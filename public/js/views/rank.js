...
import { esc, signed, num, strategyFriendly, effortLabel, intervalLabel, marketLabel, engineBadge, tickersShort, thresholdLabel } from '../format.js';
...
function table(rows) {
  ...
  return `<div class="table-wrap"><table>
    <thead><tr><th>순위</th><th>닉네임</th><th>전략</th><th>기준</th><th>effort</th><th>주기</th><th>시장</th><th>종목</th><th>기간</th>
    ...
    <tbody>${rows.map((r) => `
      <tr class="clickable rank-${r.rank}" data-run="${r.id}">
        ...
        <td>${engineBadge(r.engine)} ${esc(strategyFriendly(r.strategy))}</td><td>${esc(thresholdLabel(r.threshold))}</td><td>${esc(effortLabel(r.effort))}</td><td>${esc(intervalLabel(r.interval_days))}</td>
...
export function render(root, onOpenRun) {
  root.innerHTML = `<h2>랭킹</h2>
    ...
    <div class="card">${filterBar(['market', 'engine', 'execution', 'strategy', 'threshold', 'effort', 'intervalDays', 'sort', 'period', 'bestPerUser'], filters)}<div id="rank-table"></div></div>`;
