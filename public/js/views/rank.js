...
import { esc, signed, num, effortLabel, intervalLabel, marketLabel, engineBadge, tickersShort, thresholdLabel, legacyTag } from '../format.js';
...
function table(rows) {
  ...
  return `<div class="table-wrap"><table>
    <thead><tr><th>순위</th><th>닉네임</th><th>확신 기준</th><th>effort</th><th>주기</th><th>시장</th><th>종목</th><th>기간</th>
    ...
    <tbody>${rows.map((r) => `
      <tr class="clickable rank-${r.rank}" data-run="${r.id}">
        ...
        <td>${engineBadge(r.engine)} ${esc(thresholdLabel(r.threshold))}<span class="hint">${esc(legacyTag(r.strategy))}</span></td><td>${esc(effortLabel(r.effort))}</td><td>${esc(intervalLabel(r.interval_days))}</td>
...
export function render(root, onOpenRun) {
  root.innerHTML = `<h2>랭킹</h2>
    <p class="lead">누가 몇 % 확신 기준으로 가장 높은 수익을 냈는지 봅니다. 기간이 다르면 시장 상황이 달라지므로, 공정 비교는 <b>보유 대비 초과수익</b> 정렬이나 기간 필터를 쓰세요.</p>
    <div class="card">${filterBar(['market', 'engine', 'execution', 'threshold', 'effort', 'intervalDays', 'sort', 'period', 'bestPerUser'], filters)}<div id="rank-table"></div></div>`;
