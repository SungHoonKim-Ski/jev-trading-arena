...
import { esc, meta, pct, signed, num, usd, effortLabel, intervalLabel, thresholdLabel } from '../format.js';
...
function pivot(rows, rowValues, colOf, colValues) {
  return rowValues.map((rv) => colValues.map((c) => {
    const group = rows.filter((r) => thresholdLabel(r.threshold) === rv && String(colOf(r)) === String(c));
    const n = group.reduce((acc, r) => acc + r.runs, 0);
    if (n === 0) return null;
    const value = group.reduce((acc, r) => acc + r[metric] * r.runs, 0) / n;
    return { value, sub: `${n}회 실행`, title: `${criterion(rv)} · ${c}: ${METRICS[metric][0]} ${METRICS[metric][1](value)} (${n}회)` };
  }));
}

/** '80%' → '기준 80%', 이전 규칙은 그대로 */
const criterion = (label) => (label === thresholdLabel(null) ? label : `기준 ${label}`);

function headline(rows) {
  const best = [...rows].sort((a, b) => b[metric] - a[metric])[0];
  return `<div class="tiles">
    <div class="tile"><div class="k">최고 조합 (${esc(METRICS[metric][0])})</div><div class="v">${esc(criterion(thresholdLabel(best.threshold)))}</div>
...
async function load(root) {
  ...
  try {
    // 집계 대상은 '오를까?' 질문으로 한 실행만 (3지선다·5단계 이전 기록은 제외)
    const rows = await api(`/api/stats?${qs({ ...filters, strategy: 'noul' })}`);
    ...
    const rowLabels = [...m.thresholds.map(thresholdLabel), ...(rows.some((r) => r.threshold == null) ? [thresholdLabel(null)] : [])]
      .filter((label) => rows.some((r) => thresholdLabel(r.threshold) === label));
    out.innerHTML = `${headline(rows)}
      <div class="grid-2">
        <div class="card"><h3 style="margin-top:0">확신 기준 × effort</h3><div id="hm-effort"></div></div>
        <div class="card"><h3 style="margin-top:0">확신 기준 × 매매 주기</h3><div id="hm-interval"></div></div>
      </div>
      <div class="card"><h3 style="margin-top:0">조합별 상세</h3>${comboTable(rows)}</div>`;
    const fmt = METRICS[metric][1];
    const effortCells = pivot(rows, rowLabels, (r) => r.effort, efforts);
    const intervalCells = pivot(rows, rowLabels, (r) => intervalLabel(r.interval_days), intervals);
    const maxAbs = Math.max(0.0001, ...[...effortCells, ...intervalCells].flat().filter(Boolean).map((c) => Math.abs(c.value)));
    const rowNames = rowLabels.map(criterion);
    heatmap(out.querySelector('#hm-effort'), { rows: rowNames, cols: efforts.map(effortLabel), cells: effortCells, format: fmt, maxAbs });
    heatmap(out.querySelector('#hm-interval'), { rows: rowNames, cols: intervals, cells: intervalCells, format: fmt, maxAbs });
  } catch (ex) {
...
export function render(root) {
  root.innerHTML = `<h2>전략 분석</h2>
    <p class="lead">Jev에게 "오를까?"를 물었을 때, 몇 % 확신에서 사고파는 게 가장 잘 버는지 전체 실행 기록을 모아 비교합니다. 파란색은 매수 후 보유보다 나은 성과, 빨간색은 못한 성과입니다.</p>
