import { z } from 'zod';
import { EFFORTS, INTERVALS, STRATEGIES } from '../config.ts';
...
const MAX_YEARS = 10;
const MAX_TICKERS = 5;
const INTERVAL_DAYS = INTERVALS.map((i) => i.days);
...
export const createRunSchema = z.object({
  ...
  intervals: z.array(z.number().int().refine((d) => INTERVAL_DAYS.includes(d), '지원하지 않는 매매 주기')).min(1, '매매 주기를 1개 이상 선택하세요'),
}).superRefine((v, ctx) => {
  const pattern = v.market === 'KR' ? KR_TICKER : US_TICKER;
  v.tickers.forEach((t, i) => {
    if (!pattern.test(t)) ctx.addIssue({ code: 'custom', path: ['tickers', i], message: `${t}: ${v.market === 'KR' ? '6자리 종목코드(예: 005930)' : '미국 티커(예: AAPL)'} 형식이 아닙니다` });
  });
...
export const rankQuerySchema = z.object({
  market: z.enum(['KR', 'US']).optional().or(z.literal('').transform(() => undefined)),
...
export const collectSchema = z.object({
  market: z.enum(['KR', 'US']),
  tickers: z.array(z.string().trim().toUpperCase().regex(/^(\d{6}(\.(KS|KQ))?|[A-Z^][A-Z0-9.\-]{0,9})$/, '종목 형식이 올바르지 않습니다')).min(1).max(10),
});
